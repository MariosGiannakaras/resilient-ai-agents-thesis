...
The adapter layer adds protocol-v2 state semantics that normal SB3 persistence
does not provide by itself: CPU-only execution, branch-local virtual RNG state,
DQN replay persistence, PPO legal completed-update boundaries, deterministic
state fingerprints and continuation to an absolute interaction target without
resetting the learning schedule.
"""
...
def _restore_global_rng(state: Mapping[str, Any]) -> None:
    ...
    torch.set_rng_state(
        torch.tensor(state["torch_cpu"], dtype=torch.uint8, device="cpu")
    )
...
def _ppo_boundary(model: Any) -> str:
    rollout = model.rollout_buffer
    if int(model.num_timesteps) == 0 and int(rollout.pos) == 0 and not bool(rollout.full):
        return "initial"
    if bool(rollout.full) and int(rollout.pos) == int(rollout.buffer_size):
...
def _counter_snapshot(model: Any, *, method_id: str) -> Mapping[str, Any]:
    ...
    if method_id == "dqn":
        ...
        counters["rollout_boundary"] = _ppo_boundary(model)
        counters["rollout_buffer_size"] = int(model.rollout_buffer.buffer_size)
    return counters
...
class SB3ScientificStateAdapter:
    ...
    def export_state(self) -> Mapping[str, Any]:
        ...
        state: dict[str, Any] = {
            ...
            "configuration": self.configuration,
            "model_zip_b64": _b64(_save_model_bytes(self.model)),
            "replay_buffer_b64": None,
            "rng_state": self._rng_state,
            "action_space_rng_state": self._action_space_rng_state,
            "counters": _json_copy(
    ...
    def restore_state(self, state: Mapping[str, Any]) -> None:
        ...
        loaded = algorithm.load(
            io.BytesIO(_unb64(state["model_zip_b64"], field="model_zip_b64")),
            env=None,
            device="cpu",
            force_reset=False,
        )
        if self.method_id == "dqn":
            ...
            if state["replay_buffer_b64"] is not None:
                raise ValueError("PPO scientific state must not contain a replay buffer")
            boundary = state["counters"].get("rollout_boundary")
            if boundary == "completed-update":
                loaded.rollout_buffer.pos = loaded.rollout_buffer.buffer_size
                loaded.rollout_buffer.full = True
            elif boundary != "initial":
                raise ValueError("invalid saved PPO rollout boundary")

        self.model = loaded
        self._rng_state = _json_copy(state["rng_state"], field="SB3 RNG state")
        self._action_space_rng_state = (
            _json_copy(state["action_space_rng_state"], field="action-space RNG state")
            if state["action_space_rng_state"] is not None
            else None
        )
        _restore_action_space_rng(self.model, self._action_space_rng_state)
        self._validate_boundary()
    ...
    def learn_to_total_interactions(
        ...
        *,
        ...
        if self.model.get_env() is None:
            ...
            # A newly constructed VecEnv/Monitor must perform its own reset before
            # the first step. Exact mid-episode environment cloning is a separate
            # environment-state responsibility; the adapter owns learner state.
            self.model.set_env(self.environment_factory(), force_reset=True)
        with self._rng_scope():
            self.model.learn(
