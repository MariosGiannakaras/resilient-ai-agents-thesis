"""Backend-owned, preview-only exploratory Study projection for T-528.

This adapter deliberately materializes only a DEVELOPMENT StudyRecipe for
StudyService.preview(). It cannot create or execute a study, and it never reads
or uses the final held-out roots/layouts.
"""
...
class DesktopExploratoryPreviewModel:
    ...
    _FINAL_PATH = Path("configs/protocols/protocol-v2.0-final.json")
    ...
    def __init__(self, *, repo_root: Path) -> None:
        ...
        self._final = self._load_json(self._FINAL_PATH)
    ...
    def preview(
        ...
        *,
        ...
        if not 1 <= root_count <= self._MAX_PREVIEW_ROOTS:
            raise ValueError("root_count must be within the non-final preview pool")
        if not 1 <= layout_count <= self._MAX_PREVIEW_LAYOUTS:
            raise ValueError("layout_count must be within the non-final preview pool")
    ...
    def _recipe(
        ...
        *,
        ...
        phase_a_final = self._mapping(self._final.get("phase_a"), "final.phase_a")
        phase_b_final = self._mapping(self._final.get("phase_b"), "final.phase_b")
        ...
        roots = [
            {
                "root_id": f"t528-preview-r{index:02d}",
                "preview_only": True,
        ...
        phase_a_execution = {
            "training_interaction_budget": int(
                phase_a_final["training_interaction_budget"]
            ),
            "probe_interaction_indices": list(
                phase_a_final["probe_interaction_indices"]
            ),
            "episodes_per_probe": int(phase_a_final["episodes_per_probe"]),
            "task": {
        ...
        phase_b_execution = {
            "interaction_budget_per_branch": int(phase_b_final["horizon"]),
            "prefix_interactions": int(
                phase_b_final["common_nominal_no_learning_prefix_interactions"]
            ),
            "preview_only": True,
        }

        return StudyRecipe(
            recipe_id="t528-exploratory-preview",
            protocol_version="protocol-v2.0-development-preview",
    ...
    def _load_json(self, relative: Path) -> Mapping[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"cannot read exploratory preview authority: {path}") from exc
