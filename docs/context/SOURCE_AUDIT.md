...
|---|---|---|---|---|
| `GiannakarasMariosThesisApplication.pdf` | Official primary document | Academic identity, exact titles, official broad objective and examples | High trust for visible content; scanned one-page document; source identity explicitly confirmed by the user | Private GitHub file present; replacement not required; repository SHA-256 pending local calculation by Codex after clone |
