...
| OQ-MOD-001 | Do all three selected capability regimes survive final post-pilot freeze? | T-310 selects F0 frozen/C0 continual common tabular Q-learning plus R0 frozen rectangular robust value iteration for implementation/pilots; correctness, fairness, tuning, feasibility, and distinct empirical value still require T-312/pilots. | Final model set/protocol. | Yes. | T-312 + pilots + T-412 freeze. | Implement only the selected bounded set; remove a role if its documented gate fails. |
