...
| OQ-ENV-001 | Which GridWorld implementation is selected? | The bounded parity/feasibility comparison is complete; the project-owned path is smaller and more direct while MiniGrid adds translation, dependency/platform, and tag-level license uncertainty. The formal ADR is pending. | Before full environment implementation. | Yes. | `T-211` ADR using completed prototype evidence. | Use shared contracts in `src/resilient_agents/`; do not create a second environment interface. |
...
| OQ-MOD-002 | Is a robust-MDP comparator retained? | Citation-ready evidence now supports the conceptual role, but environment-assumption fit, exact method, scientific necessity, and sufficiency for the eventual claims are unresolved. | Final model set. | Only for that comparator. | `T-310` evidence/feasibility comparison; upstream promotion under `T-311` only if the retained claims need more support. | Treat as a conditional candidate; do not infer changepoint-recovery superiority from robust-MDP results. |
