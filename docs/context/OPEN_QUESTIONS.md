...
| OQ-ENV-002 | What exact uncertainty mechanisms, severities, onset/timing, duration, and combinations are used? | `pilot-v0.1` predeclares minimal in-set/maximal out-of-set action remaps plus dyadic single-factor diagnostics and a between-episode-block changepoint; these are diagnostic, not final severities. | Final protocol. | Yes. | Pilot evidence + literature refresh. | Keep the bounded pilot matrix; freeze or amend final values only after T-410/T-411. |
...
| OQ-MET-001 | Which operational metrics receive final primary, secondary, and diagnostic roles and parameter values? | Schema-v1 is known-answer validated and `pilot-v0.1` fixes a predeclared window/tolerance/stability sensitivity grid over matched episode-return curves; final roles/aggregation/intervals require pilots/freeze. | Final protocol. | Yes. | T-410 pilots + T-412 freeze. | Preserve full curves and all schema-v1 components; no favorable post-outcome parameter selection or composite score. |
