...
| REQ-APP-014 | The repository root must contain `run_app.bat` as the normal one-click Windows launcher for the local application. A standalone executable is not required. The launcher must be kept current whenever the application entry point, launch command, dependency bootstrap, or relevant path changes. | Explicit user direction, 2026-08-26. | CONFIRMED | From a supported Windows checkout, double-clicking root-level `run_app.bat` starts the validated application through the locked `uv` environment without requiring the user to type commands; paths resolve relative to the repository root, scientific/application logic is not duplicated in the launcher, startup failures are actionable, and launcher behavior is revalidated whenever launch mechanics change. |
...
| REQ-UI-014 | Onboarding/help remains lightweight: use native/lightweight Streamlit mechanisms and local completion state; do not add accounts/new persistence or a heavyweight custom JavaScript/DOM tour framework without demonstrated need. | Explicit user direction and scope restraint, 2026-08-04. | CONFIRMED | Implementation adds minimal complexity and does not duplicate scientific logic or create a new frontend subsystem. |
