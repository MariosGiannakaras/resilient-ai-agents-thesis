...
- **Current scientific gate: `T-527` BLOCKED on the retained one-time DEC-055 physical sizing failure; `T-526A` and `T-526` remain COMPLETE.** The reviewed authority executed once: tuning completed, but sizing failed closed at the SB3 reset-observation type boundary. No rerun/resume is authorized.
...
- DEC-055 remains the valid pre-outcome authority. Its tuning stage completed 180/180 and mechanically selected `q-c06`, `sarsa-c06`, `dqn-c05`, `ppo-c06`, `dyna-c03` and the 8,192-interaction budget. Sizing retained 97/240 Phase-A units and 192/480 matched sets before one infrastructure failure; final methods/layouts/horizon/severities/roots/statistics remain unfrozen and DEC-056 was not created.
...
Retain and independently review the DEC-055 valid-complete tuning and valid-failed sizing evidence. The exact blocker is the Frozen DQN multi-episode reset boundary: the project GridWorld reset returns a tuple observation, while the SB3 2.9.0 MultiDiscrete inference validator requires an object with `.shape`. Do not resume or rerun DEC-055. A future run requires a new explicit pre-outcome authority and reviewed type-boundary fix. Keep #95 at 7/10, T-528 blocked and the final reserve untouched.
