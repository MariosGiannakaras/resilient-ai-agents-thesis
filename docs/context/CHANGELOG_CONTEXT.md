...
- Adopted the Codex `/goal` command as the canonical long-horizon startup wrapper: it continues across successive dependency-valid bounded scopes until a genuine user/external-machine/scientific/protocol/access/review gate is reached, rather than stopping at routine task boundaries. A separate startup `/plan` remains unnecessary because the repository already owns the roadmap/task ledger; plan mode is reserved for a specific task whose approach genuinely needs investigation.
