...
- Preserved `excluded` as an analysis/inclusion status layered over the retained execution outcome; run finalization/publication no longer accepts it as a terminal execution status.
- Extended clean-source provenance to detect untracked non-output files without exposing their names; generated/local `results/**` and `artifacts/**` remain allowed, while untracked code/config/data/scripts or other repository inputs make automatic publication fail closed.
