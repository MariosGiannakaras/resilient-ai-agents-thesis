...
- **State:** `T-526 BLOCKED`; `T-526A BLOCKED` after its DEC-053 physical attempt passed recovery 30/30 but Phase B failed closed at 8/240. All 30 historical learner fingerprints/restores/derived invariants passed; the 12 DQN/PPO raw mismatches were retained transport differences and native raw identities remained exact. The next planned SARSA set could not enter the no-learning prefix because the exact checkpoint retains a deferred on-policy update. One infrastructure/lifecycle failure is retained; no rerun/resume/relaxation is authorized.
...
- **Exact next action:** obtain a new explicit scientific/lifecycle decision for the exact SARSA deferred-update boundary. Do not discard or silently resolve the update, rerun/resume the retained v0.2 attempt, overwrite either v0.2 directory, or start T-527. The successful 30/30 recovery and partial 8/240 Phase-B evidence must remain immutable.
...
- [ ] BLOCKED `T-526A` — DEC-053 recovery passed, but the retained Phase-B attempt exposed an unresolved exact SARSA boundary.
  ...
  - Blocker: SARSA `t526-r01/gw-l1-a` retains a deferred update `{state=[1,5], action=down, reward=-0.1, next_state=[1,6]}`. The no-learning prefix cannot resolve it without choosing the next on-policy action and applying the pending update, while discarding it would mutate the exact checkpoint semantics. A new pre-outcome lifecycle decision is required; DEC-053 authorizes no rerun or resume.
