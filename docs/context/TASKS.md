...
- **Package:** DEC-048/050 protocol-v2 methodology, completed DEC-051 study-first backend reconstruction and DEC-052's narrow T-526 checkpoint-materialization/Phase-B recovery amendment. DEC-042/047 and candidate v1.1 remain auditable predecessor design. DEC-049 controls the later new-framework frontend rebuild.
- **Project:** **4/8** master milestones complete (#87: 1, 2, 4, 5).
- **Current task:** `T-526` / recovery sub-gate `T-526A`.
- **State:** `T-526 BLOCKED after valid one-time Phase-A execution`; `T-526A IN_PROGRESS` under explicit DEC-052 non-outcome-driven recovery authority. The original five-file bundle remains immutable. The amendment freezes unchanged-source deterministic materialization of only the 30 selected `gw-l1` states, a mandatory 30/30 exact checkpoint/learner identity barrier, and a pilot-only one-interaction prefix plus ten-interaction no-reset FN/FD/AN/AD lifecycle for all eight already-predeclared conditions. Implementation/tests must be committed, pushed and green on PR #92 before physical recovery may execute.
...
- **Exact next action:** finish/validate the DEC-052 implementation checkpoint, push it, require green Repository and Protocol-v2 pilot checks at the PR #92 head, then execute the committed physical Windows recovery/Phase-B entrypoint once from a clean checkout. Phase B remains prohibited unless all 30 reconstructed checkpoint and learner identities exactly match the immutable rows.
...
- [ ] BLOCKED `T-526` — Run bounded environment-discrimination + method/severity/CPU feasibility pilots on the validated Windows machine.
  ...
  - Recovery resolution in progress: DEC-052 explicitly preserves the original bundle/conclusions and permits only deterministic payload materialization under Phase-A source compatibility, exact matching to every authoritative row and a complete 30/30 barrier. The separate amendment config freezes the already-declared eight conditions and a pilot-only `1` nominal no-learning prefix + `10` post-boundary interactions per branch with no reset (`11 < 12` shortest path). T-526 remains blocked until the reviewed implementation executes physically and its recovery/Phase-B evidence validates.

- [ ] IN_PROGRESS `T-526A` — Implement and physically validate DEC-052's non-outcome-driven T-526 Phase-B recovery/amendment path.
  - Depends on: retained one-time T-526 Phase-A evidence at `results/pilots/protocol-v2-feasibility-v0.1/`.
  - Authority: explicit scientific review accepted DEC-052. It supersedes only the narrow one-time wording that blocked deterministic state materialization; it does not authorize replacement evidence, changed roots/seeds/methods/candidates, final-reserve access or outcome-driven tuning.
  - Pre-execution gate: amendment/config/implementation/tests/docs must form one clean pushed PR #92 checkpoint with green Repository and Protocol-v2 pilot checks before the new physical entrypoint may run.
  - Acceptance: original five files remain byte-exact; relevant Phase-A code/config/lock has no diff from `5198dbe...`; 30/30 reconstructed payloads exactly reproduce pre-existing checkpoint and learner identities; only then execute and validate all 240 matched sets / 960 branches / 9,600 post-boundary interactions in separate immutable evidence locations.
