...
- **State:** `T-526 BLOCKED`; `T-526A BLOCKED after its one authorized physical recovery attempt failed the exact barrier`. Reviewed recovery authority head `5e784d31729ad09c40f2633f3d1682896e624317` executed on native Windows. Q-Learning and SARSA for `t526-r01/gw-l1-a` exactly matched. DQN reproduced the authoritative learner-state SHA-256 and checkpoint byte size, but its reconstructed checkpoint-envelope SHA-256 was `7b385564...` rather than authoritative `f2da03f3...`. The runner retained the failure and stopped at 2/30 exact matches; Phase B did not start. The original five-file bundle remains byte-exact.
...
- **Exact next action:** independent scientific/recovery review of the retained failed DEC-052 attempt. Do not rerun recovery, relax the exact barrier, alter serialization/seeds/configuration or execute Phase B without a new explicit non-outcome-driven authority. T-527 remains blocked.
...
- [ ] BLOCKED `T-526` — Run bounded environment-discrimination + method/severity/CPU feasibility pilots on the validated Windows machine.
  ...
  - Recovery result: DEC-052 preserved the original bundle/conclusions and required exact matching under unchanged Phase-A sources. Its one authorized physical attempt stopped on `dqn/t526-r01/gw-l1-a`: checkpoint byte size and learner-state SHA matched, but checkpoint-envelope SHA did not. Recovery evidence is retained separately; Phase B has 0/240 matched sets and 0 branch interactions. No retry or relaxed barrier is authorized.

- [ ] BLOCKED `T-526A` — DEC-052 deterministic T-526 checkpoint recovery did not pass its exact physical barrier.
  ...
  - Gate: a new explicit scientific/recovery decision is required before any retry or amendment. Do not treat the matching learner digest as permission to weaken DEC-052's required checkpoint identity.
