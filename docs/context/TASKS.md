...
- **Current task:** `T-534` is **IN_PROGRESS / merge-pending** on PR #107. The fresh-main experiment-first implementation, targeted checks and deterministic 1366×768/1440×900 review are objectively accepted; canonical pre-merge reconciliation, final exact-head CI, squash merge and merged-main verification remain.
...
- **Exact next action:** reconcile issue #104 and active docs against the accepted PR #107 implementation, run final exact-head CI on the documentation-inclusive head, verify current `main` has not advanced, then squash-merge PR #107 if all objective gates remain green. After merge, verify `main` and only then mark T-534 complete. Keep `T-610` separately BLOCKED throughout.
...
- [ ] IN_PROGRESS `T-534` — **Clean protocol-v2.1 PySide6 UI rebuild from current `main`.**
  ...
  - Implementation: PR #107 / `feat/t-534-experiment-first-ui`, created from current `main` and still zero commits behind it at pre-merge reconciliation.
  ...
  - Objective review findings were fixed narrowly, including stale onboarding test behavior, Qt deferred-delete status overlap, laptop Run layout pressure, condition-aware pairing, truthful method status, primary live facts and full registered Evidence lineage.
  - Pre-merge acceptance state: implementation and visual acceptance are satisfied; canonical issue/docs reconciliation and one final documentation-inclusive exact-head CI pass remain before squash merge.
