...
- **Last validated point:** The current WP5/WP6 implementation and evidence are preserved as a technical baseline, but the user has explicitly not accepted the current application, agent choices/behavior, experiments/runs, or resulting presentation as the final thesis state.
- **Uncommitted work:** None
- **Exact next action:** Do not start `T-700` or any `T-700+` work. Accept and execute only user-directed pre-writing refinements to the application, agents, experiments/runs, analysis, or evidence. After the user indicates that those refinements are satisfactory, explicitly ask whether they approve entering WP7. Only an unambiguous current-session affirmative response unlocks WP7/WP8.
...
- **Current gate state: NOT APPROVED.** On 2026-08-27 the user explicitly stated that the current application, agent choices/behavior, experiments/runs, and related results are not yet satisfactory as the final thesis state and that further refinement is required before writing.
- Existing checked WP5/WP6 tasks remain preserved technical milestones/evidence; they are **not** evidence that the user accepts those choices or outputs as final thesis design/deliverables.
- Before any `T-700+` work, the agent must first complete whatever pre-writing refinements the user explicitly requests. Checked earlier tasks may be reopened, superseded, amended, or supplemented by stable new task IDs when the requested refinement requires it.
- If a requested refinement changes scientific design after frozen evidence exists, preserve the existing frozen evidence and history. Use an explicit amendment/new protocol version and new Run-IDs as required; never silently mutate old finalized evidence to make it match the revised design.
- When, and only when, the user indicates that the pre-writing application/agent/experiment/result state is satisfactory, the agent must ask explicitly: **“Do you approve starting WP7 thesis-writing/defense work now?”**
...
- Until that affirmative answer is received, the agent must stop at this gate and must not start, draft, research for, generate artifacts for, mark `READY`/`IN_PROGRESS`, or otherwise advance any `T-700+` task.
