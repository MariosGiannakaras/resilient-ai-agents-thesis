...
- **Current task:** `T-527` IN_PROGRESS under DEC-056 pre-outcome correction authority; `T-526` / `T-526A` remain COMPLETE.
- **State:** DEC-055 tuning is immutable valid-complete at 180/180 and selected `q-c06`, `sarsa-c06`, `dqn-c05`, `ppo-c06`, `dyna-c03` plus 8,192 interactions. Sizing-v0.1 is immutable valid-failed at 97/240 and 192/480. DEC-056 narrowly corrects the SB3 tuple/array facade and enforces DEC-055's existing native-update horizon rule, then authorizes exactly one fresh sizing-only v0.2 attempt after commit/push/green CI/clean-head review.
...
- **Exact next action:** finish DEC-056 focused validation, commit/push the pre-run authority, require both checks green and a clean exact PR head, then run the sizing-only v0.2 Windows entrypoint exactly once. Keep #95 at 7/10, T-528 blocked and final reserve untouched until successful freeze.
...
- [x] `T-526` — Run bounded environment-discrimination + method/severity/CPU feasibility pilots on the validated Windows machine.
  ...
  - Technical diagnosis: SB3 `model.save()` persists `data.start_time` and process-sensitive human-readable class metadata outside the scientific fingerprint; canonical ZIP wrapping preserves those bytes. Focused tests prove raw-byte inequality with exact historical/derived identities and exact restores, and prove online/target/optimizer/replay/counter/RNG perturbations fail. Full audit: `docs/research/T526_SB3_SCIENTIFIC_CONTINUATION_IDENTITY_AUDIT.md`.
  - DEC-053 physical result: recovery passed 30/30 with zero learner-fingerprint mismatches, 12 retained raw SB3 envelope mismatches and exact byte sizes; Q-Learning completed all eight first-unit Phase-B conditions. The next SARSA condition failed before prefix execution because its exact checkpoint has a deferred on-policy update. Phase B retained 8/240 matched sets, 32/960 branches, eight prefix and 320/9,600 post-boundary interactions, then stopped with one infrastructure/lifecycle failure.
  - DEC-054 audit/authority: the Phase-A driver stops after the final counted `observe()` and performs no extra `act()`. Five SARSA sources retain exactly one valid deferred nonterminal backup; the existing bootstrapped-truncation path establishes the same behavior-policy/bootstrap-only-action semantics. Settlement derives separate quiescent deployment-start states with zero environment interactions before the fresh prefix. Q-Learning/DQN/PPO/Dyna-Q+ and one SARSA source are verified no-ops.
  - DEC-054 physical result: clean reviewed authority `14002a47763991234a1d0623f27330a895f348f0` passed both required checks before execution. Settlement validated 30/30 states, five non-no-op SARSA settlements, 25 no-ops, deterministic replay/round trip and zero additional environment interactions. Fresh Phase-B v0.3 completed 240/240 matched sets, 960 branches, 240 common prefixes and 9,600 post-boundary interactions with zero scientific/infrastructure failures. Independent integrity/lineage validation passes; no final reserve was accessed.
...
- [x] `T-526A` — DEC-054 boundary settlement and fresh Phase-B v0.3 physically validated.
  ...
  - DEC-053 paths: `configs/protocols/protocol-v2-t526-recovery-phase-b-v0.2.json`, `src/resilient_agents/protocol_v2_t526_recovery_v02.py`, `scripts/run_protocol_v2_t526_recovery_phase_b_v02_windows.ps1`.
  - Physical evidence: `results/pilots/protocol-v2-feasibility-v0.1-recovery-v0.2/` is valid-complete at 30/30 (33 hash-covered files, 4,760,652 bytes); `results/pilots/protocol-v2-feasibility-phase-b-v0.2/` is a valid failed attempt at 8/240 (3 hash-covered files, 24,957 bytes).
  - DEC-054 paths: `docs/decisions/DEC-054_PHASE_A_INTERACTION_BUDGET_SETTLEMENT.md`, `configs/protocols/protocol-v2-t526-boundary-settlement-phase-b-v0.3.json`, `src/resilient_agents/protocol_v2_boundary_settlement.py`, `src/resilient_agents/protocol_v2_t526_boundary_phase_b_v03.py`, `scripts/run_protocol_v2_t526_boundary_settlement_phase_b_v03_windows.ps1`.
  - Physical authority gate: clean reviewed head `14002a47763991234a1d0623f27330a895f348f0` matched remote/PR #92 and both Repository/Protocol-v2 checks were green before execution.
  - Settlement evidence: `results/pilots/protocol-v2-feasibility-boundary-settlement-v0.1/` is valid-complete at 30/30, with 33 hash-covered files, 4,755,471 bytes, 21.289 wall-seconds, five non-no-op SARSA states and zero settlement interactions/failures.
...
- [ ] IN_PROGRESS `T-527` — Fair tuning, precision/runtime sizing, statistical freeze and machine-readable protocol-v2 firewall.
  ...
  - Retained DEC-055 blocker: after a later Frozen DQN deployment episode reset, `reset_gridworld_branch_episode()` returned the project `(x, y)` tuple and `SB3PhaseBBranchDriver` passed it directly to `model.predict`; SB3 2.9.0 MultiDiscrete observation validation accessed `.shape` and raised `AttributeError`. The incomplete v0.1 matrix justified no final freeze and cannot be resumed or rerun. DEC-056 is a separate pre-outcome correction/retry authority, not a conclusion from partial performance.
  - DEC-056 pre-outcome correction authority/config/runner: `docs/decisions/DEC-056_T527_SIZING_OBSERVATION_BOUNDARY_RETRY.md`, `configs/protocols/protocol-v2-t527-sizing-retry-v0.2.json`, `scripts/run_protocol_v2_t527_sizing_v02_windows.ps1`. It preserves the project `(x, y)` scientific state and converts only SB3-facing observations to exact declared-dtype arrays; it also records/enforces the already-frozen method-native update-opportunity criterion. It authorizes exactly one fresh sizing-only v0.2 attempt after reviewed-head gates. Tuning-v0.1 and sizing-v0.1 are immutable inputs; #95 remains 7/10.
