...
- **Current task:** `T-610` is **IN_PROGRESS under DEC-062 recovery**. The first authorized Study stopped after 216/603 completed jobs at a deterministic SARSA checkpoint-boundary implementation defect and is preserved as immutable failed/incomplete history. The user explicitly authorized a complete, non-mixed replacement execution on 2026-09-02; T-611/T-612/T-613 and WP7 remain unauthorized.
...
- **Pre-final readiness:** COMPLETE scientifically, for the accepted application, and for active-tree hygiene. The committed final gate remains sealed. DEC-062 authorizes only the infrastructure correction and fresh replacement T-610 execution while the first Study stays unfinalized and ineligible for downstream evidence.
...
- **Exact next action:** complete the bounded DEC-062 recovery PR, verify exact-head CI and merge it; synchronize clean Windows `main`; rerun the full read-only 603-job preflight; then create `protocol-v2.1-final--t610-recovery-01` from the corrected single source commit and execute it from zero. Never resume or mix the historical attempt. Stop after objective T-610 completion with T-611 next.
...
- [ ] IN_PROGRESS `T-610` — Execute frozen protocol-v2.1 final matrix through the accepted study-first execution path.
  ...
  - Exact next action: merge the bounded correction/evidence-preservation PR after focused tests and exact-head CI; synchronize clean Windows `main`; require the read-only preflight to reproduce the unchanged recipe and 603-job plan; then create and execute the replacement from zero using the unchanged authorization guard. Stop after T-610 completion with T-611 next.
