...
5. **The uncertainty taxonomy should remain bounded.** NovGrid demonstrates many novelty types, while action-robust and observation-robust work show that action failure and observation corruption are technically distinct. The thesis should select only the few that map directly to the official topic and research question.
6. **Algorithm complexity is not automatically scientific value.** ESCP, Bayesian change detection and dynamic-regret algorithms establish upper-end references; they do not require reproduction in a bounded undergraduate thesis.
