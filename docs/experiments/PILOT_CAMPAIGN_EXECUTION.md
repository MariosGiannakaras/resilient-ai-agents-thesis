...
`src/resilient_agents/pilot_campaign.py` and `scripts/run_pilot_campaign.py` are the fail-closed execution path for `T-410`. The driver runs only `pilot-v0.1`, only from durable `main`, and resumes by validating deterministic run identities rather than overwriting output.
...
7. derive `PV01-PILOT-ANALYSIS` through the validated analysis pipeline and write the campaign state under `results/campaigns/pilot-v0.1/`.
