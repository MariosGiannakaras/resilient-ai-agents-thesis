...
Each child/whole experiment preserves the resolved protocol/scenario, all root/derived seeds, source/runtime provenance, Q checkpoint or robust-plan checksum, event log, episode-return curves, execution outcome/reason, metric sensitivity summaries, manifest, and checksums. `T-401` enforces this protocol through the resumable CLI/core path documented in `HEADLESS_RUNNER.md`; `T-402` provides reproducible summaries; and `T-410` uses the predeclared durable-main campaign path in `PILOT_CAMPAIGN_EXECUTION.md`.
