> Source: https://arxiv.org/pdf/2508.07790
...
′ 
P ′ . We say that the policy π is (strictly) dominant in the RMDP 
MR if it (strictly) dominates every other policy π′ ∈ Π\{π}. Next, we say that a policy is best-effort if there is no other policy that dominates it. Definition 6 (Best-effort). A policy π ∈ Π for the RMDP MR is best-effort if there is no π′ ∈ Π such that π′ >P π. We denote by ΠBE ⊆ Π the set of all best-effort policies. 
...
Remark 1. For clarity and due to space constraints, all proofs are provided in Sect. A. 
...
derivative at the minimizing (resp. maximizing) P ⋆ ∈ P . In this section, we complete the characterization by showing that any policy that satisfies these conditions up to this uniqueness is also best-effort. Theorem 3 formalizes this non-trivial result. For conciseness, we defer the preliminaries needed for the proof of Theorem 3 to Sect. A.4. Theorem 3 (Computing ORBE policies). Let Π⋆ = argmaxπ∈Π ρπP be the optimal robust policies. Pick two transition functions P (1), P (2) ∈ P such that P (1) ̸= P (2) and, for all s̄ ∈ S, the line gs̄(λ) = λP 
(1) s̄ +(1−λ)P (2) 
...
In the proof, presented in Sect. A.4, we show that there always exists a policy π⋆ ∈ Π⋆ 
...
9 Acknowledgments This research is supported by the EPSRC grant EP/Y028872/1, Mathematical Foundations of Intelli-gence: An “Erlangen Programme” for AI. 
...
′ 
P or vice versa for all P ∈ P . 
Due to the smoothness and pointwise comparability of these functions, and the compactness of the uncertainty set 
3For details on such solution functions for parametric MDPs, we refer to Junges et al. (2024). 
...
(i) s̄ ) and ∇vZ 
π′ 
P,s̄(P (i) s̄ ), i = 1, 2, in the direction 
...
P,s̄ and Zπ′ 
P,s̄. For convenience, we simplify the rational function Zπ 
P,s̄(Ps̄) defined by Eq. (10) as 
fπ P,s̄(x) = 
a+ b⊤x 
1− c⊤x , (18) 
