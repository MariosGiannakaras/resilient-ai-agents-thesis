...
RIGHTS_UNVERIFIED: Final[str] = (
    "Rights/license not independently verified; local research use only; "
    "do not commit or redistribute until checked."
)
...
class Source:
    ...
    source_url: str
    access_note: str
    rights_status: str = RIGHTS_UNVERIFIED
...
SOURCES: Final[tuple[Source, ...]] = (
    Source(
        ...
        "Xiangyu Peng; Julia Kim; Mark Riedl",
        2022,
    ...
    Source(
        ...
        "Andrew Lefrancq; Laurent Orseau; Shane Legg",
        2017,
    ...
    Source(
        ...
        "Carolin Benjamins; Theresa Eimer; Frederik Schubert; André Biedenkapp; "
    ...
    Source(
        ...
        "https://papers.nips.cc/paper_files/paper/1990/file/"
        "d9fc5b73a8d78fad3d6dffe419384e70-Paper.pdf",
    ...
    Source(
        ...
        "Christian Alexander Steinparz; Thomas Schmied; Fabian Paischer; "
    ...
    Source(
        ...
        "Adapting the Behavior of Reinforcement Learning Agents to Changing Action Spaces "
    ...
    Source(
        ...
        "Restarted Bayesian Online Change-point Detection for Non-Stationary Markov "
        "Decision Processes",
    ...
    Source(
        ...
        "Robust Deep Reinforcement Learning against Adversarial Perturbations on State "
        "Observations",
        "Huan Zhang; Hongge Chen; Chaowei Xiao; Bo Li; Mingyan Liu; Duane Boning; "
        "Cho-Jui Hsieh",
        2020,
        "Research paper / arXiv preprint",
...
MANUAL_ACQUISITION: Final[tuple[dict[str, str], ...]] = (
    ...
    },
    {
        ...
        "official_url": "https://research.tue.nl/en/publications/"
        "adaptive-reinforcement-learning-lean-and-dynamic-agents-for-robus/",
...
def validate_pdf(path: Path) -> None:
    if path.stat().st_size < 1024:
        raise ValueError(
            f"Downloaded file is unexpectedly small: {path.stat().st_size} bytes"
        )
    with path.open("rb") as handle:
...
def write_manifest_atomic(payload: dict[str, object]) -> None:
    """Write a complete manifest before atomically replacing the previous one."""
...
def quarantine(path: Path, reason: str) -> Path:
    ...
    while target.exists():
        target = path.with_name(
            f"{path.name}.{reason}.{suffix}.{counter}.quarantine"
        )
        counter += 1
    path.replace(target)
    return target
...
def merged_manual_acquisition(
    ...
    merged.extend(
        record for source_id, record in existing.items() if source_id not in known_ids
    )
    return merged
...
def main() -> int:
    ...
    for source in SOURCES:
        ...
        try:
            checksum, downloaded_now = prepare_source_file(
                source, destination, previous
            )
            same_revision = bool(previous and previous.get("sha256") == checksum)
            retrieved_at = (
                str(previous.get("retrieved_at_utc"))
                if same_revision and previous and previous.get("retrieved_at_utc")
                else utc_now()
            )
            record = {
                **asdict(source),
                ...
                "full_text_review_complete": bool(
                    same_revision
                    and previous
                    and previous.get("full_text_review_complete", False)
                ),
            }
    ...
    # Preserve user-acquired and future records that are not managed by this script.
    records.extend(
        record
        for source_id, record in existing_records.items()
        if source_id not in curated_ids
    )
    ...
    output = {
        ...
        "policy": (
            "Lawful open-access, author-provided or institutional sources only; "
            "no paywall bypass. Rights status must be verified separately before "
            "repository commit or redistribution."
        ),
        "sources": records,
        "failures": failures,
        "manual_acquisition": merged_manual_acquisition(manifest),
    }
    write_manifest_atomic(output)
